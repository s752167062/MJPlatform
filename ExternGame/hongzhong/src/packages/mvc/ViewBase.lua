...
function ViewBase:createResoueceNode(resourceFilename)
    ...
    self.resourceNode_ = display.newCSNode(resourceFilename)
...
function ViewBase:createResoueceBinding(binding)
    ...
    for nodeName, nodeBinding in pairs(binding) do
        ...
        local node = self:getDescendantsByName(self.resourceNode_ , nodeName)
...
function ViewBase:getDescendantsByName(node , name)
    ...
    for key, var in ipairs(node:getChildren()) do
        local child =  self:getDescendantsByName(var , name)
        if child then
            return child
        end
    end
