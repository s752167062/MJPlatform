...
local UI_MemberOperation = class("UI_MemberOperation", ex_fileMgr:loadLua(PATH_CLUB_LUA.."LUIView"))

local _w = nil
function UI_MemberOperation:ctor(data)
    self.app = data.app
    self.param = data.param
    self.root = display.newCSNode("club/UI_MemberOperation.csb")
    self.root:addTo(self)

    UI_MemberOperation.super.ctor(self)
    _w = self._childW
...
function UI_MemberOperation:onEnter()
    -- body
    CCXNotifyCenter:listen(self,function(obj,key,data) self:onSearchMember(data) end, "S2C_onSearchPlayer")

end

function UI_MemberOperation:onExit()
    CCXNotifyCenter:unListenByObj(self)
end

function UI_MemberOperation:update(t)
    
end

function UI_MemberOperation:initUI()
    -- body
    self.title          = _w["title"]
...
--主设置
function UI_MemberOperation:setMainUI()
...
function UI_MemberOperation:onClick(_sender)
...
--搜索
function UI_MemberOperation:searchMember()
...
function UI_MemberOperation:onSearchMember(data)
    cclog("UI_MemberOperation:onSearchClub") 
