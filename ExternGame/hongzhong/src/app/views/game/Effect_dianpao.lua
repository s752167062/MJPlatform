local Effect_dianpao = class("Effect_dianpao",function() 
    return cc.Node:create()
end)

function Effect_dianpao:ctor(scene, player)
    self.root = display.newCSNode("effect/eff_dianpao.csb")
    self.root:addTo(self)
    self.mInterval = 0
    local ac = cc.CSLoader:createTimeline("effect/eff_dianpao.csb");
    ac:play("a0",false)
    self:runAction(ac)
...
function Effect_dianpao:update(t)
    if self.isDead then
        return
    end
...
function Effect_dianpao:onExit()
    ex_timerMgr:unRegister(self)
    self:unregisterScriptHandler()
end

return Effect_dianpao