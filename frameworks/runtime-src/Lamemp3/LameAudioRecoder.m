...
static LameAudioRecoder * instance = nil;
@implementation LameAudioRecoder


+ (LameAudioRecoder *) getInstance{
     if (instance == nil) {
          instance = [[LameAudioRecoder alloc] init];
     }
...
- (void)StartRecoderLame:(NSString *)filenamee Directory:(NSString *) directory CallBack:(void(^)(BOOL))callfunc{
    ...
    [FileHelper setBaseDirectory:directory];//设置默认的读取目录
     ...
     if (range.location > 0 ) {
          //mp3 校验
          if(![filenamee hasSuffix:@".mp3"]){
              [self alertMsg:@" 录制仅支持 .mp3 文件的录制"];
              return ;
          }
          //修改
     ...
     if (self.recorder.isRecording){//录音中
         ...
         if (callfunc != nil) {
          ...
          self.recorder = [[AVAudioRecorder alloc]initWithURL:[NSURL fileURLWithPath:self.recordFilePath]
                                                     settings:[LameVoiceConverter GetAudioRecorderSettingDict]
                                                        error:nil];
...
- (void)EndRecoderLame:(void(^)(BOOL))callfunc{
     if (self.recorder.isRecording){//录音中
         ...
         //开始转换格式
         NSString *mp3Path = [FileHelper GetPathByFileName:self.recordFileName ofType:@"mp3"];
//         #wav 转MP3 then
         if ([LameVoiceConverter ConvertWavToMp3:self.recordFilePath mp3SavePath:mp3Path]){
             // mp3 文件完成 ！！
             if (callfunc != nil) {
                 callfunc(true);
             }
...
- (void)PlayAmr:(NSString *)filenamee Directory:(NSString *) directory CallBack:(void(^)(BOOL))callfunc{
     ...
     //转码
     if ([LameVoiceConverter ConvertAmrToWav:amrPath wavSavePath:convertedPath]){
