...
ClubSelectGameView.RESOURCE_FILENAME = __platformHomeDir .."ui/layout/ClubView/ClubSelectGameView.csb"
...
function ClubSelectGameView:onEnter()
    ...
    self.checkBoxGroup = {
        {obj = self.cb_majiang, state = false, gameType = externGameMgr.GameType_majiang},
        {obj = self.cb_puke, state = false, gameType = externGameMgr.GameType_puke},
        {obj = self.cb_zipai, state = false, gameType = externGameMgr.GameType_paohuzi},
    }
...
function ClubSelectGameView:onBack()
    viewMgr:close("ClubView.ClubSelectGame_view")
end
...
function ClubSelectGameView:setCheckBoxGroup(ctype)
    ...
    self.gtype = ctype
    hallSendMgr:sendGameList(tab.gameType, 1)
end
...
function ClubSelectGameView:onClickGame(index)
    ...
    if common and common.data then
        ...
        eventMgr:registerEventListener("HallProtocol.onAddClubGame", function(res)
                cclog("xxx >")
                if res.isSucc then
                    hallSendMgr:sendClubData(self.clubId)
                    self:onBack()

              
                    -- gatewaySendMgr:sendCheckGotoGame(0, common.data.product)
                    -- platformExportMgr:setEnterParams(platformExportMgr.epType_gotoGameClub, {clubId = self.clubId, clubSecndId = data.clubSecndId})
                end
            end, self)
        hallSendMgr:sendAddClubGame(self.clubId, common.data.product)
    end
end
