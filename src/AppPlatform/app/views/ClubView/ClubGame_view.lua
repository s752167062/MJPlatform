...
function ClubGameView:onEnter()
    ...
    playerIcon:setIcon(gameConfMgr:getInfo("userId"), gameConfMgr:getInfo("headUrl"))
...
function ClubGameView:recvCommonList(res)
    ...
    for i, v in ipairs(list) do
        ...
        if i == 1 and canSet then
            ...
            item:findChildByName("txt_id"):setVisible(false)
            if #list == 1 then
            ...
            local txt_id = item:findChildByName("txt_id")
            txt_id:setString(string.format("(ID: %d)",v.clubSecndId))
