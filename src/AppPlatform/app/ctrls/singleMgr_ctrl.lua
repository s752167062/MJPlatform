...
function SingleMgr:_intLocks()
	self._locks = {}
	self._locks["login"] = {value = false,change = false}
end
...
function SingleMgr:register(name,reName,params)
	...
	_G[reName] = self._singles[name]
	dump("register single "..name)
end

function SingleMgr:unRegister(name)
	self._singles[name] = nil
	_G[name] = nil
end

function SingleMgr:clear()
	for k,v in pairs(self._singles) do
		if k then
			_G[k] = nil
		end
	end
